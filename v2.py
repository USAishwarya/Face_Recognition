...
frame_skip = 3               # inference processes every nth frame
...
# keep references to threads so we can check/join if desired
...
# ---------------- Model load ----------------
# small opt: disable OpenCL usage in OpenCV (sometimes helps consistency)
cv2.ocl.setUseOpenCL(False)

try:
    model = load_model(MODEL_PATH)
    print("Model loaded")
    # warm-up model to avoid first-predict delay
    try:
...
# ---------------- Face recognition (producer/consumer) ----------------
def _capture_loop():
    """Capture frames and push latest to frame_queue. Runs in capture thread."""
    ...
    # set a reasonable resolution to reduce CPU load
        ...
        while recognition_active:
            ...
            try:
                ...
                try:
                    ...
                    pass
            # small sleep to yield (capture runs as fast as camera)
            time.sleep(0.01)
    finally:
        cap.release()

def _inference_loop():
    """Consume frames, run detection+inference (single-threaded), update preview_label."""
    global recognition_active
    face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
    local_frame_count = 0

    while recognition_active or not frame_queue.empty():
        try:
        ...
        local_frame_count += 1
        # optionally skip frames for inference; still render preview
        do_infer = (local_frame_count % frame_skip == 0)

        # run detection + inference only when do_infer True
        if do_infer:
            try:
                ...
                for (x, y, w, h) in faces:
                    ...
                    # draw rectangle and name (no confidence)
                    cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                    cv2.putText(frame, name, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)

                    # log attendance in same (inference) thread (serial)
                    try:
                        log_attendance(name)
                    except Exception as e:
                        print("log_attendance thread error:", e)
            except Exception as e:
                print("Detection error:", e)

        # convert frame to PIL Image and update preview_label on main thread
        try:
            rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            pil = Image.fromarray(rgb)
            # create thumbnail for GUI (keeps memory reasonable)
            pil.thumbnail((640, 480))
            imgtk = ImageTk.PhotoImage(image=pil)
            # schedule update on the main GUI thread
            root.after(0, lambda im=imgtk: preview_label.configure(image=im) or setattr(preview_label, "image", im))
        except Exception as e:
            print("Preview update error:", e)

    # when finishing clear preview optionally
    try:
        blank = Image.new("RGB", (320, 240), (30, 30, 30))
        im_blank = ImageTk.PhotoImage(blank)
        root.after(0, lambda: preview_label.configure(image=im_blank) or setattr(preview_label, "image", im_blank))
    except:
        pass

def start_recognition():
    """Starts capture and inference threads if not already active."""
    global recognition_active, _capture_thread, _infer_thread
    ...
    _infer_thread = Thread(target=_inference_loop, daemon=True)
    _infer_thread.start()

    recognition_status_var.set("Running")

def stop_recognition():
    """Stops recognition. Threads are daemonic and will exit."""
    global recognition_active
    recognition_active = False
...
ctk.CTkLabel(recognition_frame, text="Camera feed will open embedded below.", font=("Arial", 10), text_color="#7F8C8D").pack(anchor="w", padx=12)

# ---------------- EMBEDDED PREVIEW LABEL ----------------
preview_container = ctk.CTkFrame(recognition_frame, fg_color="transparent")
preview_container.pack(padx=12, pady=8, fill="both", expand=False)
preview_label = tk.Label(preview_container, bd=0)   # will be updated with PhotoImage from inference thread
preview_label.pack()
...
# ensure on close recognition threads are stopped
def on_closing():
    global recognition_active
    recognition_active = False
    # small pause to let camera threads release
    time.sleep(0.25)
    root.destroy()
